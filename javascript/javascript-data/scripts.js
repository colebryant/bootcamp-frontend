...
// QUESTION ONE:
// Adds total items in commit array under the payload property to variable totalCommits and console.logs them
...
// QUESTION TWO: 
// Creates variables for each event type, creates a function to loop through the object and increment each variable if they match the event type
// Finally, consoles logs the number of events of each type
// Also, solves the problem using another method

let numPushEvent = 0;
let numPullRequestEvent = 0;
let numDeleteEvent = 0;
let numIssueCommentEvent = 0;
let numCreateEvent = 0;
...
addEvents(githubData);

console.log("Push Event:", numPushEvent, "PullRequestEvent:", numPullRequestEvent, "DeleteEvent:", numDeleteEvent, "IssueCommentEvent:", 
numIssueCommentEvent, "CreateEvent:", numCreateEvent);

// other way of doing it

const sumEvent = (eventType) => {
  let totalEvents = 0;
  for (let i = 0; i < githubData.length; i++) {
      if (eventType === githubData[i].type) {
          totalEvents++;
      }
  }
  console.log(`${eventType}: ${totalEvents}`);
}

sumEvent("PushEvent");
sumEvent("PullRequestEvent");
sumEvent("DeleteEvent");
sumEvent("IssueCommentEvent");
sumEvent("CreateEvent");
...
for (let i = 0; i < githubData.length; i++) {
  if (githubData[i].type === "PullRequestEvent") {
    if (!listOfUsers.includes(githubData[i].payload.pull_request.user.login)) {
      listOfUsers.push(`${githubData[i].payload.pull_request.user.login}`);
    }
  }
}
...
let client_side_mastery = 0;
let bangazon_llc = 0;
let vps_setup = 0;
let brenda_snack_cake_store = 0;

for (let i = 0; i < githubData.length; i++) {
  if (githubData[i].actor.login === "stevebrownlee" && githubData[i].repo.name === "nashville-software-school/client-side-mastery") {
    client_side_mastery++;
  } else if (githubData[i].actor.login === "stevebrownlee" && githubData[i].repo.name === "nashville-software-school/bangazon-llc") {
    bangazon_llc++;
  } else if (githubData[i].actor.login === "stevebrownlee" && githubData[i].repo.name === "stevebrownlee/vps-setup") {
    vps_setup++;
  } else {
    brenda_snack_cake_store++;
  }
}

console.log("client_side_mastery", client_side_mastery, "bangazon_llc", bangazon_llc, "vps_setup", vps_setup, "brenda_snack_cake_store", brenda_snack_cake_store);
...
const sumCommits = (eventType) => {
  let totalCommits = 0;
  for (let i = 0; i < githubData.length; i++) {
      if (eventType === githubData[i].type && "commits" in githubData[i].payload) {
        totalCommits += githubData[i].payload.commits.length;
      }
  }
  console.log(eventType + totalCommits);
}

sumCommits("PushEvent");
sumCommits("PullRequestEvent");
sumCommits("DeleteEvent");
sumCommits("IssueCommentEvent");
sumCommits("CreateEvent");

// QUESTION SIX / SEVEN:

for (let i = 0; i < githubData.length; i++) {
  if (githubData[i].actor.login === "stevebrownlee" && "pull_request" in githubData[i].payload) {
    console.log(githubData[i].payload.pull_request.base.repo.language);
  }
}

// from console.log above, javascript was most affected by steve